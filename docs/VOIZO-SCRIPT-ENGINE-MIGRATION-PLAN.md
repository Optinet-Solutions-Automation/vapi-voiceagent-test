...
The source repo verifies every engine change against a **local harness**: `scripts/verify-script-engine.mjs` — a mock OpenAI router (canned intents keyed on magic phrases), a VAPI control-URL stub capturing `say`/`add-message`/`end-call`, and a dev server driven by synthetic webhook posts. Its 42 assertions cover routing, arming, menus (members/else/statements/marks/debts), the backchannel gate, the interruption arbiter, watchdog idempotency, wait-box silence paths, goodbye handling, and the observer pass; run instructions are in its header. Port it alongside the engine (the pattern: temp rows with `tmp_` keys → drive webhooks → assert events + control messages → cleanup in `finally`). Adapt to vitest if preferred, but keep the end-to-end shape — unit tests alone missed every serverless bug this engine ever had. Ghost-portal `is_test` campaigns are the staging equivalent for real PSTN calls.
